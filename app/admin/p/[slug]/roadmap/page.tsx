...
import { ArrowLeft, Plus, ChevronUp, MessageCircle } from "lucide-react";
...
export async function generateMetadata({
  ...
  return { title: `Manage Roadmap — ${project.name} — Roadmap Nexus`, description: `Manage roadmap items for ${project.name}.` };
}
...
export default async function ProjectAdminRoadmapPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
    ...
    <div className="p-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-[#5c6878]">Manage roadmap items for {project.name}.</p>
        </div>
            ...
            {items.map((item) => (
              <tr key={item.id} className="transition hover:bg-[#fafbfc]">
                ...
                <td className="px-4 py-3">
                  <span className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-semibold ${statusStyles[item.status] || ""}`}>
                ...
                <td className="px-4 py-3">
                  <span className="inline-flex items-center gap-1 text-sm text-[#8a95a3]">
                    <ChevronUp className="h-3.5 w-3.5" />{item.votes}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <span className="inline-flex items-center gap-1 text-sm text-[#8a95a3]">
